...
- **Common Strategies & dlt Mapping**:
  - **Cursor-based**:
    ...
    - Identify: Where is the *next* cursor value found in the response? (e.g., `pagination.next_cursor`, `meta.next`, `links.next.href`,`_paging._nextCursor`). Map this to `cursor_path`.
    - Identify: What is the *query parameter name* to send the next cursor? (e.g., `cursor`, `page_token`, `after`,`_cursor`). Map this to `cursor_param`.
    - Identify: What is the parameter for page size? Map to `_limit` (set this in `endpoint.params`, not the paginator dict).
    - dlt `type`: `cursor`
  - **Offset-based**:
